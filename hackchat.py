        ...
        while True:
            ...
            if result["cmd"] == "chat" and not result["nick"] == self.nick:
                for handler in list(self.on_message):
                    try:
                        handler(self, result["text"], result["nick"], result["trip"], self.online_users)
                    except:
                        handler(self, result["text"], result["nick"], 'NaNNaN', self.online_users)
            elif result["cmd"] == "onlineAdd":
                self.online_users.append(result["nick"])
                for handler in list(self.on_join):
                    try:
                        handler(self, result["nick"], result["trip"])
                    except:
                        handler(self, result["nick"], 'NaNNaN')
                ...
                for handler in list(self.on_whisper):
                    try:
                        handler(self, result["text"], result["from"], result["trip"])
                    except:
                        handler(self, result["text"], result["from"], 'NaNNaN')
